...
import { LogOut, RefreshCw, LayoutDashboard, BarChart3, GitCompare } from 'lucide-react';
...
export default function Dashboard({ config, onDisconnect }: Props) {
  const [tab, setTab] = useState<'monitoring' | 'comparison' | 'benchmark'>('monitoring');
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex gap-1 bg-slate-800/80 rounded-xl p-1 border border-slate-700/50">
            ...
            <button onClick={() => setTab('comparison')}
              className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${tab === 'comparison' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:text-white'}`}>
              <GitCompare className="w-3.5 h-3.5" />Comparaison Live
            </button>
      ...
      <main className="flex-1 overflow-hidden p-4">
        {tab === 'monitoring' && (
          <div className="h-full flex flex-col gap-3">
            <div className="shrink-0"><GlobalCounter /></div>
        ...
        {tab === 'comparison' && (
          <div className="h-full overflow-y-auto pr-1 pb-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2"><GlobalCounter /></div>
              <div className="col-span-2"><LiveComparison /></div>
              <LatencyChart />
              <MachineDistribution />
            </div>
          </div>
